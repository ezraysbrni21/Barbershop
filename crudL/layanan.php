<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Dashboard</title>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.js"></script>
    <style type="text/css">
        .wrapper{
            width: 1000px;
            margin:auto
        }
        .page-header h2{
            margin-top: 0;
        }
    </style>
    <script type="text/javascript">
        $(document).ready(function(){
            $('[data-toggle="tooltip"]').tooltip();
        });
    </script>
</head>
<body>
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="pull-left">Informasi Pegawai</h2>
                        <a href="add.php" class="btn btn-success pull-right">Tambah Baru</a>
                    </div>
                    <table border=1 cellpadding=6>
                        <tr>
                            <th>No</th>
                            <th>Nama Produk</th>
                            <th>Harga Layanan</th>
                            <th>Deskripsi Layanan</th>
                            <th>Diskon Layanan</th>
                            <th>Layanan</th>
                            <th>Pengaturan</th>
                        </tr>
                    <?php
                    include "config.php";
                    $no = 1;
                    $sql = "select * from layanan order by id asc";
                    $result = mysqli_query($conn, $sql);
                    while($row = mysqli_fetch_assoc($result)) { ?>
                    <tr>
                        <td><?= $no++ ?></td>
                        <td><?= $row['nama_produk'] ?></td>
                        <td><?= $row['harga_layanan'] ?></td>
                        <td><?= $row['deskripsi_layanan'] ?></td>
                        <td><?= $row['diskon_produk'] ?></td>
                        <td><?= $row['layanan'] ?></td>
                        <td>
                            <a href="edit.php?id=<?= $row['id'] ?>" class="btn btn-warning">Ubah</a>
                            <a onclick="return confirm('Anda Yakin?');" href="delete.php?id=<?= $row['id'] ?>" class="btn btn-danger">Hapus</a>
                        </td>
                    </tr>
                    <?php }
                    // if($result = mysqli_query($conn, $sql)){
                    //     if(mysqli_num_rows($result) > 0){
                    //         echo "<table class='table table-bordered table-striped'>";
                    //             echo "<thead>";
                    //                 echo "<tr>";
                    //                     echo "<th>#</th>";
                    //                     echo "<th>Nama Produk</th>";
                    //                     echo "<th>Layanan</th>";
                    //                     echo "<th>Harga Layanan</th>";
                    //                     echo "<th>Deskripsi Layanan</th>";
                    //                     echo "<th>Diskon Layanan</th>";
                    //                     echo "<th>Pengaturan</th>";
                    //                 echo "</tr>";
                    //             echo "</thead>";
                    //             echo "<tbody>";
                    //             while($row = mysqli_fetch_array($result)){
                    //                 echo "<tr>";
                    //                     echo "<td>" . $row['id'] . "</td>";
                    //                     echo "<td>" . $row['nama_produk'] . "</td>";
                    //                     echo "<td>" . $row['layanan'] . "</td>";
                    //                     echo "<td>" . $row['harga_layanan'] . "</td>";
                    //                     echo "<td>" . $row['deskripsi_layanan'] . "</td>";
                    //                     echo "<td>" . $row['diskon_layanan'] . "</td>";
                    //                     echo "<td>";
                    //                         echo "<a href='read.php?id=". $row['id'] ."' title='View Record' data-toggle='tooltip'><span class='glyphicon glyphicon-eye-open'></span></a>";
                    //                         echo "<a href='update.php?id=". $row['id'] ."' title='Update Record' data-toggle='tooltip'><span class='glyphicon glyphicon-pencil'></span></a>";
                    //                         echo "<a href='delete.php?id=". $row['id'] ."' title='Delete Record' data-toggle='tooltip'><span class='glyphicon glyphicon-trash'></span></a>";
                    //                     echo "</td>";
                    //                 echo "</tr>";
                    //             }
                    //             echo "</tbody>";
                    //         echo "</table>";
                    //         // Free result set
                    //         mysqli_free_result($result);
                    //     } else{
                    //         echo "<p class='lead'><em>No records were found.</em></p>";
                    //     }
                    // } else{
                    //     echo "ERROR: Could not able to execute $sql. " . mysqli_error($link);
                    // }

                    // Close connection
                    ?>
                    
                    </table>
                </div>
            </div>
        </div>
    </div>
</body>
</html>